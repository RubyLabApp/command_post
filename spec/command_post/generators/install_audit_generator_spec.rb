...
require "generators/command_post/install_audit/install_audit_generator"

RSpec.describe CommandPost::Generators::InstallAuditGenerator, type: :generator do
  ...
  describe "migration template" do
    let(:template_path) do
      File.join(described_class.source_root, "create_command_post_audit_entries.rb.tt")
    end
    ...
    it "creates command_post_audit_entries table" do
      content = File.read(template_path)
      expect(content).to include("create_table :command_post_audit_entries")
    end
    ...
    it "includes indexes" do
      content = File.read(template_path)
      expect(content).to include("add_index :command_post_audit_entries, :resource")
      expect(content).to include("add_index :command_post_audit_entries, :action")
      expect(content).to include("add_index :command_post_audit_entries, :created_at")
    end
  end
end
