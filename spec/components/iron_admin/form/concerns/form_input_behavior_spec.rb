...
require_relative "../../../../../app/components/iron_admin/form/concerns/form_input_behavior"

RSpec.describe IronAdmin::Form::Concerns::FormInputBehavior do
  let(:test_class) do
    Class.new do
      include IronAdmin::Form::Concerns::FormInputBehavior
  ...
  describe "#effectively_disabled?" do
    ...
    context "when disabled is false but field is readonly" do
      let(:readonly_field) { IronAdmin::Field.new(:email, readonly: true) }
    ...
    context "when field uses proc for readonly" do
      ...
      let(:field) { IronAdmin::Field.new(:email, readonly: ->(user) { !user.admin? }) }
  ...
  describe "#field_readonly?" do
    ...
    context "when field is not readonly" do
      let(:editable_field) { IronAdmin::Field.new(:email, readonly: false) }
  ...
  describe "#theme" do
    it "returns the configuration theme" do
      component = test_class.new
      expect(component.theme).to eq(IronAdmin.configuration.theme)
    end
  end

  describe "attr_readers" do
    it "exposes field" do
      field = IronAdmin::Field.new(:email)
